...
// 1. جلب تفاصيل الطلب مع خط السير (GET)
export async function GET(_: Request, { params }: { params: { id: string } }) {
  ...
  const o = await prisma.order.findUnique({
    ...
    include: {
      ...
      payments: true,
      timelines: { orderBy: { createdAt: 'desc' } }, // جلب سجل التتبع الزمني مرتباً
    },
  });
...
// 2. تحديث حالة الطلب وتسجيلها في OrderTimeline عبر Transaction (PATCH)
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    // تنفيذ عملية التحديث والسجل الزمني في Transaction واحدة متكاملة
    const [updatedOrder] = await prisma.$transaction([
      ...
      prisma.orderTimeline.create({
        data: {
          ...
          adminId: u.id, // حفظ معرف الأدمن الذي قام بالتغيير
        },
      }),
    ]);
