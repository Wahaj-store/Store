...
export async function hashPassword(password: string) {
  ...
  return new Promise<string>((resolve, reject) =>
    crypto.scrypt(password, salt, 64, { N: 16384, r: 8, p: 1 }, (e, key) =>
      e ? reject(e) : resolve(`s2$${salt}$${key.toString('hex')}`)
    )
  );
}
...
export async function verifyPassword(password: string, stored: string) {
  try {
    ...
    // توافق مع الهاشات القديمة (V6)
    const legacy = await new Promise<Buffer>((resolve, reject) =>
      crypto.scrypt(password, 'wahaj-salt', 64, (e, key) => e ? reject(e) : resolve(key))
    );
...
export async function createSession(userId: string) {
  const token = await new SignJWT({ sub: userId, aud: 'admin', typ: 'admin-session' })
    ...
    .sign(getSecret());
  cookies().set('wahaj_session', token, {
    httpOnly: true,
...
export async function getUser() {
  ...
  try {
    ...
    // إذا تم تغيير كلمة المرور بعد إصدار التوكن → ارفض الجلسة
    if (user.passwordChangedAt) {
      const tokenIssuedAtMs = payload.iat * 1000;
      if (tokenIssuedAtMs < user.passwordChangedAt.getTime()) return null;
    }
